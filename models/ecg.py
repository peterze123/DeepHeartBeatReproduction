...
"""
Modified from DeepHeartBeat: https://github.com/laumerf/DeepHeartBeat
to support dependencies on Collab with help of LLM
"""
class Encoder(Layer):
...
class ECGModel(Model):
    ...
    def fit(self, train_data, val_data):
      ...
      train_dataset = tf.data.Dataset.from_generator(train_dataset_generator, output_types=dataset_output_types, output_shapes=dataset_output_shapes)
      val_dataset = tf.data.Dataset.from_generator(val_dataset_generator, output_types=dataset_output_types, output_shapes=dataset_output_shapes)
      ...
      train_log_dir = './self_trained_models/train'
      val_log_dir = './self_trained_models/validation'
      ...
      for metric in train_metrics:
          self._train_metrics[metric] = tf.keras.metrics.Mean(name=metric)
      
      # setup validation metrics
      self._validation_metrics = dict()
      val_metrics = ['reconstruction_error', 'heart_rate']
      for metric in val_metrics:
          self._validation_metrics[metric] = tf.keras.metrics.Mean(name=metric)
      ...
      while count < patience and epoch <= max_epochs:
          ...
          # train
          for step, (times, sequences) in enumerate(train_dataset.take(steps_per_epoch)):
          ...
          for metric in self._validation_metrics.values():
              metric.reset_states()
